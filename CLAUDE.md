...
Three services (`docker-compose.yml`): **`minecraft-quest`** (the child's container: ttyd web
terminal + the quest + Minecraft server), **`portal`** (parent dashboard), and
**`terminal-shell`** (a tiny nginx static app — the installable PWA shell for the terminal).
`minecraft-quest` and `portal` communicate **only** through a shared Docker volume mounted at
`/data` (`sessao-data`) — no direct network/API call; all coordination is file-based.

Access is **browser-only** (no SSH): `docker-entrypoint.sh` runs `ttyd --base-path /app ...
su - jogador`, so each browser connection opens a fresh quest session as `jogador`. ttyd's
HTTP Basic Auth (`--credential jogador:$JOGADOR_SENHA`) is the password gate, served over
HTTPS by Traefik. On `TERMINAL_DOMAIN`, Traefik routes `/app` → ttyd (router `questterm`,
priority 100) and everything else → `terminal-shell` (router `questshell`, priority 1); the
shell's `index.html` embeds the terminal in a full-screen `<iframe src="/app/">`. Minecraft
(25565) is the only host-published port.

**PWA:** both web entrypoints are installable on Android. `portal` serves its own
`/manifest.webmanifest`, `/sw.js` and `/icon-{192,512}.png` (FastAPI routes; app "VGTUX
Painel"). `terminal-shell` serves the same set as static files (app "VGTUX Aventura"); its
service worker bypasses `/app` so it never caches the live terminal/WebSocket. Icons are
pixel-art generated by `scripts/gen_icons.py` (stdlib only) into `portal/` and `terminal-shell/`.

Repo layout: `sessao.py` + `Dockerfile` + `iniciar-minecraft.sh` + `docker-entrypoint.sh`
build the quest container (root context). `portal/` and `terminal-shell/` each have their own
...
- Host port **8080 is taken by Traefik** — no web service is published on the host. All three
  join the external `dokploy-network` and are routed by Traefik **labels** in
  `docker-compose.yml`:
  - `ADMIN_DOMAIN` → `portal` (port 8080).
  - `TERMINAL_DOMAIN` → `terminal-shell` (port 80, priority 1) for everything except `/app`,
    and → `minecraft-quest` ttyd (port 7681, priority 100) for `PathPrefix(/app)`.
  - TLS via `letsencrypt` on all.
...
- Deploy via the Dokploy UI as a **Compose** app pointing at the Git repo; set the env vars
  ...
  the compose labels, don't enable both for the same host (duplicate Traefik routers) — and note
  the terminal needs the two-router split (`/app` → ttyd, rest → shell).
